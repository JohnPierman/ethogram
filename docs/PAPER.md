...
We report three negative results of independent interest. Combining the detectors' p-values,
by Fisher's method with Brown's correction or by the Šidák-corrected minimum, does not
exceed the best single component at any budget. Alerting whenever any detector ranks an
event highly is strictly dominated at equal cost and superior at equal depth, at 3.7 times
the alert volume. And a population-scope model retains one capability the entity-scope
framework lacks, so the two scopes are complementary rather than ordered.
...
unattended. We do not claim that combining evidence across detectors helps: three
combination rules are measured and none exceeds its own best component at equal cost
(section 5.4). We do not claim that population-scope conditioning is useless; it retains one
...
out-of-hours activity are as common among background events as among labelled ones. This is
the explanation for the null results the timing and volume detectors return throughout section
5.2, and it is a better explanation than misspecification: a test of a property that does not
separate labelled events from background cannot rank labelled events highly however well it is
specified. It also bounds what any amount of work on those two detectors could recover here.
