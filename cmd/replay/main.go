...
func main() {
	var (
		...
		noveltyRateOn = flag.Bool("novelty-rate", false, "add Detector V, the novelty-RATE detector: is this entity producing first-ever values at a higher rate than it historically does, K ~ BetaBinomial(m, a, b) over an hourly window. Detector I asks how improbable one novel value was, and its answer is essentially 1/n, so an account needs ~117,000 events of history before any single novel value can win an alert slot and 736 of 856 planted attack events were unreachable. This asks a scale-free question instead. Off by default until a recorded run justifies the flip. Recorded in the result")
		pairingOn     = flag.Bool("pairing", false, "replace Detector III's population co-occurrence null with its per-entity form: is this pairing novel for THIS entity, against its own history. The population form tests departure from the population norm, which the framework elsewhere disavows; it measured 18.4% of events below 1e-12 and contributed nothing to detection. Off by default until a recorded run justifies the flip. Recorded in the result")
