...
// figBudgetCurve is §1.1's figure, generated by cmd/budgetcurve from a recorded framework run
// and a recorded baselines run over the same events, and committed so that cmd/thesis stays
// deterministic and independent of results/. Regenerate with `make figure-budget-curve`, then
// `make paper`; `paper-check` is what catches forgetting the second step.
//
//go:embed figures/budget-curve.svg
var figBudgetCurve string

// figBudgetTable is the headline table under §1.1's figure, generated by the same command from
// the same recorded runs, so a number in the table and a point on the curve above it cannot
// disagree. It lists every comparator that was measured, including the four the figure has no
// room to draw. Regenerate with `make figure-budget-curve`, then `make paper`.
//
//go:embed figures/budget-table.html
var figBudgetTable string

var _ embed.FS // the embed import is required for the directive above
...
func figures() map[string]string {
	return map[string]string{
		...
		"budget-curve":           figBudgetCurve,
		"budget-table":           figBudgetTable,
